...
#model_name = "wordToVecModel"# with 6000 num_feature
...
#words_ = X_train[3]
#vector = featureVec = np.zeros((num_features,),dtype="float32")
#data_cloud = []
#index2word_set = set(model.wv.index2word)
#for word in words_:
#    if word in index2word_set:
#        data_cloud.append(model[word])
def plotHistogram():
...
cluster = DBSCAN(eps=.0001, min_samples=1000, metric='euclidean', algorithm='auto', leaf_size=30, p=None, n_jobs=-1)
cluster_data = cluster.fit(data_cloud)
print list(cluster_data.labels_)
